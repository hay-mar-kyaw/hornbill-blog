<?php 
    ...
    if(isset($_GET['category_id'])){
        ...
        $stmt=$db->prepare("SELECT blogs.id, blogs.title,blogs.content,blogs.created_at,blogs.image,users.name FROM blogs INNER JOIN users ON blogs.user_id=users.id WHERE blogs.category_id=$categoryId ORDER BY blogs.id DESC");
    }else{
        $stmt=$db->prepare("SELECT blogs.id, blogs.title,blogs.content,blogs.created_at,blogs.image,users.name FROM blogs INNER JOIN users ON blogs.user_id=users.id ORDER BY blogs.id DESC");
    }
    $stmt->execute();
                    ...
                    <div class="card my-3" data-aos="zoom-in" data-aos-duration="1000">
                        <div class="card-body p-0">
                            ...
                            <div class="content p-3">
                                ...
                                <p>
                                    <?php echo substr($blog->content,0,120)?>
